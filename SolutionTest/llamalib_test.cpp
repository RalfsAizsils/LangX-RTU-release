...
int main() {
    ...
    fs::path out_file = SCRIPT_DIR / ("result_llamalib_" + timestamp_str() + ".txt");
    std::ostringstream log;

    struct Result { std::string name, status, reason; double time = 0; };
    std::vector<Result> results;

    log << "LlamaLib C++ Test \xe2\x80\x93 " << iso_now() << "\n";
    log << "Model: " << MODEL_PATH.string() << "\n";
    log << "Note: LlamaLib does not support VLM/image processing.\n";
    log << std::string(60, '=') << "\n";

    auto t_total = Clock::now();

    // --- Setup: LLMService + LLMAgent ---
    ...
    LLMAgent* agent = new LLMAgent(service, "You are a helpful assistant. Answer concisely.");
    ...
    for (const auto& p : prompts) {
        ...
        std::string response = agent->chat(p.prompt);
